	...
	function avada_contact_info() {
		...
		if ( $phone_number || $email ) {
			$html .= '<div class="fusion-contact-info">';
				$html .= '<i class="fas fa-phone" data-fa-transform="flip-h"></i><a href="tel:'.$phone_number.'">'.$phone_number.'</a>';
				if ( $phone_number && $email ) {
					if ( 'Top' == $header_position ) {
						$html .= '<span class="fusion-header-separator">&nbsp;&nbsp;&nbsp;</span>';
					} else {
						$html .= '<br />';
					}
				}
				$html .= '<i class="far fa-envelope"></i>'.sprintf( apply_filters( 'avada_header_contact_info_email', '<a href="mailto:%s">%s</a>' ), $email, $email );
			$html .= '</div>';
		}
		return $html;
	}
